...
contract Invoker is IInvoker, Ownable, BalanceCarrier {
  ...
  mapping(address => address[]) public _liquidityProxies;
  ...
  function setLiquidityProxies(address tokenAddress, address[] calldata liquidityProxies) external  onlyFresh onlyOwner {
   ...
   console.log(liquidityProxies[0]);
  }
  ...
  function isTokenAddressRegistered(address tokenAddress) public view override returns (bool) {
    ...
    console.log("Token Addres",_liquidityProxies[tokenAddress].length);
  ...
  function removeStuckTokens(
    ...
    address to,
    uint256 amount
  ) external onlyFresh onlyOwner returns (bool) {
